...
export default class AddNewCus extends React.Component {
    ...
    updateExisting = async () => {
        ...
        const { cus_name, phone_number, city, cus_address, cus_note, docId, geoLocation } = this.state;
        this.setState({ loading: true });
        if (cus_name && city && phone_number  && docId) {
        const res = await updateCustomer(docId,cus_name, phone_number, city, cus_address, cus_note, geoLocation);
        if(res=== "true"){
    ...
    creteNew = async () => {
        const { cus_name, phone_number, city, cus_address, cus_note, geoLocation } = this.state;
        if (cus_name && city && phone_number ) {
            this.setState({ loading: true });
            const res = await saveCus(cus_name, phone_number, city, cus_address, cus_note,geoLocation);
            if(res === "true"){
                window.location.reload(false);
    ...
    render() {
        const { cus_name, phone_number, city, cus_address, cus_note, err, loading, docId, geoLocation } = this.state;
                ...
                <div className="row-main-rest-home-inpu-create">
                    <div className="input-field input-create-rest-home-full">
                        {/* <input  id="cus_address" value={cus_address}
                            onChange={(e) => { this.handleChangeText(e); }}
                            type="text" className="validate" /> */}
                            <MapPicker setGeoLocation={this.setGeoLocation} geo={geoLocation}/>
